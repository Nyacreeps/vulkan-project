...
void VulkanApplication::createInstance(const std::string& appName, const std::string& engineName) {
...
VulkanApplication::QueueFamilyIndices VulkanApplication::findQueueFamilies(
    ...
    for (const auto& queueFamily : queueFamilies) {
        ...
        if (presentSupport && !indices.presentFamily.has_value()) indices.presentFamily = i;
        if (graphicsSupport && !indices.graphicsFamily.has_value()) {
            indices.graphicsFamily = i;
            graphicsFamilyQueueCount = queueFamily.queueCount;
        }
        if (transferSupport && !indices.transferFamily.has_value()) indices.transferFamily = i;

        if (presentSupport && !graphicsSupport) indices.presentFamily = i;
        if (transferSupport && !graphicsSupport) indices.transferFamily = i;
        if (graphicsSupport && queueFamily.queueCount > graphicsFamilyQueueCount) {
            indices.graphicsFamily = i;
            graphicsFamilyQueueCount = queueFamily.queueCount;
        }
        i++;
